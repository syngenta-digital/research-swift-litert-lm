  ...
  for (const [name, backend] of [
    ...
    describe(`Engine (${name})`, () => {
      ...
      describe('Conversation', () => {
        ...
        it('throws an error if getHistory is called while busy', async () => {
          const message = {
            role: 'user',
            content: 'First message',
          };
          const p1 = conversation.sendMessage(message);
          expect(() => conversation.getHistory())
              .toThrowError(/Conversation is busy/);
          await expectAsync(p1).toBeResolved();
        });
      });
