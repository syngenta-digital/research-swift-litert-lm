...
import {ConversationConfig, Message, ContentPart, MessageLike, Tool, ToolCall} from '../conversation_config.js';
...
/**
 * Options for configuring an AutoToolConversation.
 */
export interface AutoToolConversationOptions {
  engine: Engine;
 ...
 */
export class AutoToolConversation implements ChatInterface {
  ...
  constructor(private readonly options: AutoToolConversationOptions) {
    if (options.config?.preface?.tools &&
        options.config.preface.tools.length > 0) {
      throw new Error(
          'AutoToolConversation handles tools itself. Do not provide tools via ConversationConfig.preface.tools.');
    }

    const activeTools = options.tools || [];
    for (let t of activeTools) {
      if (isWebMcpTool(t)) {
        t = {...toTool(t), execute: t.execute} as ToolWithImplementation;
      } else if (!('function' in t)) {
        const {execute, ...declaration} = t;
        t = {type: 'function', function: declaration, execute} as unknown as
            ToolWithImplementation;
      }

      const name = 'function' in t ? t.function.name : t.name;
